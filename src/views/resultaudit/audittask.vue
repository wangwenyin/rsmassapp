...
<div class="margin60">
  <el-row :gutter="10" >
  <el-col :span="2.5">
     <router-link :to="{ name:'paramSet' ,param: { rwbh: rwbh }}" >
    <el-button type="primary" v-on:click="rateAnal">比率分析</el-button>
     </router-link>
    </el-col>
  ...
  <el-table
    ...
    <el-table-column 
     ...
     <template slot-scope="scope">
       <div>
          <router-link :to="{ name:'auditdetail',params: { step: scope.row.rwzt }}"   >
            <el-button type="text" size="small">详情</el-button>
          </router-link>     
          <router-link :to="{ name:'paramSet'}"  > 
            <el-button type="text" size="small" v-if="scope.row.rwzt == '参数设定'"  >作业</el-button>
          </router-link>         
            <router-link :to="{ name:'rateAnal'}"  > 
              <el-button type="text" size="small"  v-if="scope.row.rwzt == '比率分析'">作业</el-button>
            </router-link>
       </div> 
      </template>
    </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getTask() {
      this.$http.get('src/mock/rateanal.json').then(response => {
