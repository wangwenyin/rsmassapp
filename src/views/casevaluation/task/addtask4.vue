<template>
  <div class="caseSearch" @click="isShowMating=false">
    ...
    <baidu-map class="bm-view" :style="{width: `${mapWidth}px`}" ref="baiduMap" v-if="isShow" :center="center" :zoom="zoom" @zoomend="zoomChange" :map-click="false" :scroll-wheel-zoom="true" @ready="mapReady">
      ...
      <bm-control anchor="BMAP_ANCHOR_BOTTOM_RIGHT" :offset="{width: 10, height: 50}">
           <!-- <el-button plain size="small" @click="isShowMating = !isShowMating">可比案例</el-button> -->
        <el-popover
          title="可比实例："
          width="100%"
          trigger="click">
          <div>
              <el-row>
       <el-col :span="4" v-for="o in 5"  :key="o.id"  >
         <el-card class="box-card">
            <div class="text">
               <div> 君悦阁1706 </div>
               <div> 建筑面积：76.88 </div>
               <div>交易时间：2018-04-12 </div>
               <div><b class="red">￥81350元/平方米</b></div>
                <div> <el-button  size="mini" icon="el-icon-delete"></el-button></div>
            </div>
         </el-card>
       </el-col>
         <el-col :span="4"   class="col">
          <el-card class="box-card">
             <div class="text">
               <router-link :to="{ name:'addtask5' ,param: { rwbh: rwbh }}" >
                 <el-button type="primary" >价格评估</el-button>
               </router-link>
          ...
          <el-button plain size="small" slot="reference" @click="isShowMating=false">对比案例</el-button>
        </el-popover>
      </bm-control>
      <bm-control v-if="isShowMating" anchor="BMAP_ANCHOR_BOTTOM_RIGHT" :offset="{width: 210, height: 40}" @click.native.stop>
...
<script>
  ...
  export default {
    ...
    methods: {
      handleClick() {
        this.isShowMating = !this.isShowMating
  
        // if (this.isShowMating === true) {
        //   this.isShowMating = false
        // } else {
        //   this.isShowMating = true
        // }
        console.log('this.isShowMating')
        console.log(this.isShowMating)
      },
      handle() {
